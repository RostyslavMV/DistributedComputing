...
public class UserInterface extends JFrame {
  ...
  UserInterface() {
    ...
    btnAddStation.addMouseListener(
        new MouseAdapter() {
          public void mouseClicked(MouseEvent event) {
            editMode = false;
            wayMode = false;
    ...
    comboWay.addActionListener(
        new ActionListener() {
          public void actionPerformed(ActionEvent e) {
            String name = (String) comboWay.getSelectedItem();
            currentWay = client.wayFindByName((String) comboWay.getSelectedItem());
            wayMode = true;
            wayPanel.setVisible(true);
            stationPanel.setVisible(false);
            fillWayFields();
    ...
    comboStation.addActionListener(
        new ActionListener() {
          public void actionPerformed(ActionEvent e) {
  ...
  private static void save() {
    if (editMode) {
      if (wayMode) {
        ...
        if (!client.stationUpdate(currentStation)) {
          JOptionPane.showMessageDialog(null, "Error: something went wrong!");
        }
      }
    } else {
  ...
  private static void delete() {
    if (editMode) {
      if (wayMode) {
        var list = client.stationFindByWayId(currentWay.getId());
        for (Station c : list) {
          comboStation.removeItem(c.getName());
          client.stationDelete(c);
        }
        client.wayDelete(currentWay);
        comboWay.removeItem(currentWay.getName());

      } else {
        client.stationDelete(currentStation);
  ...
  private static void fillStationFields() {
    ...
    Way cnt = client.wayFindById(currentStation.getWayId());
